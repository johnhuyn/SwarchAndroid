...
void unitCollison()
{
  ...
  for (int h = 0; h < myNetAddressList.list ().size(); h++)
  {
    for (int i = 0; i < 4; i++)
    {
      if ((xCube[h]  >= xCoord[i] - pOneCenter - 5 && xCube[h]  <= xCoord[i] + pOneCenter + 5) 
        && (yCube[h]   >= yCoord[i] - pOneCenter - 5 && yCube[h]   <= yCoord[i] + pOneCenter + 5))
      {
      ...
      {
        if( (xCube[h] - 10 > 1920 - 100) || (xCube[h] - 10 < 0) || (yCube[h] + 10 > 1200 - 100) || (yCube[h] - 10 < 0))
        {
